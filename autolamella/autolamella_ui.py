...
class UiInterface(QtWidgets.QMainWindow, UI.Ui_MainWindow):
    ...
    def connect_protocol_signals(self):
        ...
        self.comboBoxapplication_file.currentTextChanged.connect(
            self.get_protocol_from_ui
        )

        # trench
        self.doubleSpinBox_trench_lamella_height.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_trench_lamella_width.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_trench_milling_depth.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_trench_trench_height.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_trench_offset.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_trench_size_ratio.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_trench_milling_current.editingFinished.connect(
            self.get_protocol_from_ui
        )

        # notch
        self.doubleSpinBox_notch_hheight.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_notch_hwidth.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_notch_vheight.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_notch_vwidth.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_notch_depth.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_notch_distance.editingFinished.connect(
            self.get_protocol_from_ui
        )
        self.doubleSpinBox_notch_milling_current.editingFinished.connect(
            self.get_protocol_from_ui
        )
    ...
    def draw_patterns(self):
        ...
        pixelsize = (
            self.image_widget.image_settings.hfw
            / self.image_widget.image_settings.resolution[0]
        )
        self.lamella_stages = []
        ...
        default_position_lamella = (
            self.lamella_position
            if self.lamella_position is not None
            else Point(0.0, 0.0)
        )  # has the user defined a position manually? If not use 0,0
        self.lamella_position = default_position_lamella
        index = self.lamella_index.currentIndex()
        if (
            self.experiment is not None and len(self.experiment.positions) > 0
        ):  # do we have an experiment with at least one lamella
        ...
        if self.microexpansionCheckBox.isChecked():
            ...
            protocol["milling_current"] = self.settings.protocol["lamella"]["stages"][
                0
            ]["milling_current"]
            protocol["lamella_width"] = self.settings.protocol["lamella"][
                "lamella_width"
            ]
            protocol["application_file"] = self.settings.protocol["application_file"]
            mill_settings = self.get_milling_settings(protocol)

            pattern.define(protocol=protocol, point=lamella_position)
        ...
        self.settings.protocol["lamella"]["stages"][0][
            "hfw"
        ] = self.image_widget.image_settings.hfw
        self.settings.protocol["lamella"]["stages"][1][
            "hfw"
        ] = self.image_widget.image_settings.hfw
        self.settings.protocol["lamella"]["stages"][2][
            "hfw"
        ] = self.image_widget.image_settings.hfw
        self.lamella_stages = patterning._get_milling_stages(
            "lamella", self.settings.protocol, point=lamella_position
        )
        ...
        default_position_fiducial = (
            self.fiducial_position
            if self.fiducial_position is not None
            else Point(
                -self.image_widget.image_settings.resolution[0] / 3 * pixelsize, 0.0
            )
        )  # has the user defined a fiducial position?
        ...
        self.notch_stages = patterning._get_milling_stages(
            "notch", self.settings.protocol, point=notch_centre
        )
    ...
    def create_experiment(self):
        ...
        folder_path = _get_directory_ui(
            msg="Select experiment directory", path=cfg.LOG_PATH
        )
        ...
        self.log_path = os.path.join(
            self.save_path, self.experiment_name, "logfile.log"
        )
        self.lines = 0
        ...
        if self.microscope is not None:
            ...
            self.instructions_textEdit.setPlainText(
                INSTRUCTION_MESSAGES["connect_message"]
            )
            _ = message_box_ui(
                title="Next step:",
    ...
    def load_experiment(self):
        ...
        self.experiment = (
            Experiment.load(file_path) if file_path != "" else self.experiment
        )
        if file_path == "":
            return
        ...
        try:
            ...
            if self.lines != lin_len:
                for i in reversed(range(lin_len - self.lines)):
                    ...
                    if (
                        re.search("DEBUG", line_display)
                        or re.search("vispy", line_display)
                        or re.search("Unknown key", line_display)
                        or re.search("STATUS", line_display)
                    ):
                        self.lines = lin_len
                        continue
    ...
    def connect_to_microscope(self):
        ...
        self.log_path = os.path.join(self.settings.image.save_path, "logfile.log")
        self.lines = 0
        ...
        if self.experiment is not None:
            ...
            self.instructions_textEdit.setPlainText(
                INSTRUCTION_MESSAGES["create_experiment_message"]
            )
            _ = message_box_ui(
                title="Next step:",
    ...
    def update_image_message(self, add=False):
        if self.initial_setup_stage is False:
            self.instructions_textEdit.setPlainText(
                INSTRUCTION_MESSAGES["add_lamella_message"]
            )
        ...
        if add is True or len(self.experiment.positions) > 0:
            ...
            create_lamella_text = INSTRUCTION_MESSAGES["mod_lamella_message"].format(
                self.lamella_added,
                self.lamella_saved,
                self.lamella_added,
    ...
    def load_protocol(self):
        protocol_path = _get_file_ui(
            path=cfg.BASE_PATH, msg="Select protocol file", _filter="*yaml"
        )
        ...
        error_returned = check_loaded_protocol(
            self.settings.protocol, _THERMO, _TESCAN, _DEMO
        )
    ...
    def set_ui_from_protocol(self):
        ...
        self.beamshift_attempts.setValue(
            (self.settings.protocol["lamella"]["beam_shift_attempts"])
        )
        self.fiducial_length.setValue(
            (self.settings.protocol["fiducial"]["height"] * constants.SI_TO_MICRO)
        )
        self.width_fiducial.setValue(
            (self.settings.protocol["fiducial"]["width"] * constants.SI_TO_MICRO)
        )
        self.depth_fiducial.setValue(
            (self.settings.protocol["fiducial"]["depth"] * constants.SI_TO_MICRO)
        )
        self.current_fiducial.setValue(
            (
                self.settings.protocol["fiducial"]["milling_current"]
                * constants.SI_TO_NANO
            )
        )
        self.presetComboBox_fiducial.setCurrentText(
            self.settings.protocol["fiducial"].get("preset", None)
        )
        self.stage_lamella.setCurrentText("1. Rough Cut")
        self.select_stage()
        self.micro_exp_width.setValue(
            (self.settings.protocol["microexpansion"]["width"] * constants.SI_TO_MICRO)
        )
        self.micro_exp_height.setValue(
            (self.settings.protocol["microexpansion"]["height"] * constants.SI_TO_MICRO)
        )
        self.micro_exp_distance.setValue(
            (
                self.settings.protocol["microexpansion"]["distance"]
        ...
        if isinstance(self.microscope, ThermoMicroscope):
            if (
                self.comboBoxapplication_file.findText(
                    self.settings.protocol["application_file"]
                )
                == -1
            ):
                napari.utils.notifications.show_warning(
                    "Application file not available, setting to Si instead"
                )
                self.settings.protocol["application_file"] = "Si"
            self.comboBoxapplication_file.setCurrentText(
                self.settings.protocol["application_file"]
            )

        if self.comboBox_current_alignment.count() == 0:
            self.comboBox_current_alignment.addItems(
                ["Imaging Current", "Milling Current"]
            )

        protocol_alignment_current = self.settings.protocol["lamella"][
            "alignment_current"
        ]

        if protocol_alignment_current.lower() in [
            "imaging",
            "imaging current",
            "imagingcurrent",
        ]:
            self.comboBox_current_alignment.setCurrentText("Imaging Current")
        elif protocol_alignment_current.lower() in [
            "milling",
            "milling current",
            "millingcurrent",
        ]:
        ...
        # trench
        self.doubleSpinBox_trench_lamella_height.setValue(
            (self.settings.protocol["trench"]["lamella_height"] * constants.SI_TO_MICRO)
        )
        self.doubleSpinBox_trench_lamella_width.setValue(
            (self.settings.protocol["trench"]["lamella_width"] * constants.SI_TO_MICRO)
        )
        self.doubleSpinBox_trench_milling_depth.setValue(
            (self.settings.protocol["trench"]["depth"] * constants.SI_TO_MICRO)
        )
        self.doubleSpinBox_trench_trench_height.setValue(
            (self.settings.protocol["trench"]["trench_height"] * constants.SI_TO_MICRO)
        )
        self.doubleSpinBox_trench_offset.setValue(
            (self.settings.protocol["trench"]["offset"] * constants.SI_TO_MICRO)
        )
        self.doubleSpinBox_trench_size_ratio.setValue(
            (self.settings.protocol["trench"]["size_ratio"])
        )
        self.doubleSpinBox_trench_milling_current.setValue(
            (self.settings.protocol["trench"]["milling_current"] * constants.SI_TO_NANO)
        )

        # notch
        self.doubleSpinBox_notch_hheight.setValue(
            self.settings.protocol["notch"]["hheight"] * constants.SI_TO_MICRO
        )
        self.doubleSpinBox_notch_hwidth.setValue(
            self.settings.protocol["notch"]["hwidth"] * constants.SI_TO_MICRO
        )
        self.doubleSpinBox_notch_vheight.setValue(
            self.settings.protocol["notch"]["vheight"] * constants.SI_TO_MICRO
        )
        self.doubleSpinBox_notch_vwidth.setValue(
            self.settings.protocol["notch"]["vwidth"] * constants.SI_TO_MICRO
        )
        self.doubleSpinBox_notch_depth.setValue(
            self.settings.protocol["notch"]["depth"] * constants.SI_TO_MICRO
        )
        self.doubleSpinBox_notch_milling_current.setValue(
            self.settings.protocol["notch"]["milling_current"] * constants.SI_TO_NANO
        )
        self.doubleSpinBox_notch_distance.setValue(
            self.settings.protocol["notch"]["distance"] * constants.SI_TO_MICRO
        )
        self.checkBox_notch_flip.setChecked(
            bool(self.settings.protocol["notch"]["flip"])
        )
    ...
    def select_stage(self):
        ...
        self.lamella_width.setValue(
            (
                self.settings.protocol["lamella"]["stages"][index]["lamella_width"]
                * constants.SI_TO_MICRO
            )
        )
        self.lamella_height.setValue(
            (
                self.settings.protocol["lamella"]["stages"][index]["lamella_height"]
                * constants.SI_TO_MICRO
            )
        )
        self.trench_height.setValue(
            (
                self.settings.protocol["lamella"]["stages"][index]["trench_height"]
                * constants.SI_TO_MICRO
            )
        )
        self.depth_trench.setValue(
            (
                self.settings.protocol["lamella"]["stages"][index]["depth"]
                * constants.SI_TO_MICRO
            )
        )
        self.offset.setValue(
            (
                self.settings.protocol["lamella"]["stages"][index]["offset"]
                * constants.SI_TO_MICRO
            )
        )
        self.current_lamella.setValue(
            (
                self.settings.protocol["lamella"]["stages"][index]["milling_current"]
                * constants.SI_TO_NANO
            )
        )
        self.size_ratio.setValue(
            (self.settings.protocol["lamella"]["stages"][index]["size_ratio"])
        )
        self.presetComboBox.setCurrentText(
            self.settings.protocol["lamella"]["stages"][index].get("preset", None)
        )

    def get_protocol_from_ui(self):
        self.settings.protocol[
            "application_file"
        ] = self.comboBoxapplication_file.currentText()
        self.settings.protocol["lamella"]["beam_shift_attempts"] = float(
            self.beamshift_attempts.value()
        )
        self.settings.protocol["fiducial"]["height"] = float(
            self.fiducial_length.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["fiducial"]["width"] = float(
            self.width_fiducial.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["fiducial"]["depth"] = float(
            self.depth_fiducial.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["fiducial"]["milling_current"] = float(
            self.current_fiducial.value() * constants.NANO_TO_SI
        )
        self.settings.protocol["fiducial"][
            "preset"
        ] = self.presetComboBox_fiducial.currentText()
        self.settings.protocol[
            "application_file"
        ] = self.comboBoxapplication_file.currentText()
        ...
        self.settings.protocol["lamella"]["lamella_width"] = float(
            self.lamella_width.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["lamella"]["lamella_height"] = float(
            self.lamella_height.value() * constants.MICRO_TO_SI
        )

        index = self.stage_lamella.currentIndex()

        self.settings.protocol["lamella"]["stages"][index]["lamella_width"] = float(
            self.lamella_width.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["lamella"]["stages"][index]["lamella_height"] = float(
            self.lamella_height.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["lamella"]["stages"][index]["trench_height"] = float(
            self.trench_height.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["lamella"]["stages"][index]["depth"] = float(
            self.depth_trench.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["lamella"]["stages"][index]["offset"] = float(
            self.offset.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["lamella"]["stages"][index]["milling_current"] = float(
            self.current_lamella.value() * constants.NANO_TO_SI
        )
        self.settings.protocol["lamella"]["stages"][index]["size_ratio"] = float(
            self.size_ratio.value()
        )
        self.settings.protocol["lamella"]["stages"][index][
            "preset"
        ] = self.presetComboBox.currentText()
        self.settings.protocol["microexpansion"]["width"] = float(
            self.micro_exp_width.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["microexpansion"]["height"] = float(
            self.micro_exp_height.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["microexpansion"]["distance"] = float(
            self.micro_exp_distance.value() * constants.MICRO_TO_SI
        )

        self.settings.protocol["lamella"][
            "alignment_current"
        ] = self.comboBox_current_alignment.currentText().lower()

        # trench
        self.settings.protocol["trench"]["lamella_width"] = float(
            self.doubleSpinBox_trench_lamella_width.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["trench"]["lamella_height"] = float(
            self.doubleSpinBox_trench_lamella_height.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["trench"]["trench_height"] = float(
            self.doubleSpinBox_trench_trench_height.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["trench"]["depth"] = float(
            self.doubleSpinBox_trench_milling_depth.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["trench"]["offset"] = float(
            self.doubleSpinBox_trench_offset.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["trench"]["size_ratio"] = float(
            self.doubleSpinBox_trench_size_ratio.value()
        )
        self.settings.protocol["trench"]["milling_current"] = float(
            self.doubleSpinBox_trench_milling_current.value() * constants.NANO_TO_SI
        )
        self.settings.protocol["trench"]["hfw"] = self.image_widget.image_settings.hfw

        # notch
        self.settings.protocol["notch"]["hheight"] = float(
            self.doubleSpinBox_notch_hheight.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["notch"]["hwidth"] = float(
            self.doubleSpinBox_notch_hwidth.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["notch"]["vheight"] = float(
            self.doubleSpinBox_notch_vheight.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["notch"]["vwidth"] = float(
            self.doubleSpinBox_notch_vwidth.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["notch"]["depth"] = float(
            self.doubleSpinBox_notch_depth.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["notch"]["distance"] = float(
            self.doubleSpinBox_notch_distance.value() * constants.MICRO_TO_SI
        )
        self.settings.protocol["notch"]["milling_current"] = float(
            self.doubleSpinBox_notch_milling_current.value() * constants.NANO_TO_SI
        )
        self.settings.protocol["notch"]["hfw"] = float(
            self.image_widget.image_settings.hfw
        )
    ...
    def update_displays(self):
        ...
        if self.show_lamella.isChecked():
            ...
            patterns: list[list[FibsemPatternSettings]] = [
                stage.pattern.patterns
                for stage in self.trench_stages
                if stage.pattern is not None
            ]
            _draw_patterns_in_napari(
                self.viewer,
            ...
            patterns: list[list[FibsemPatternSettings]] = [
                stage.pattern.patterns
                for stage in self.notch_stages
                if stage.pattern is not None
            ]
            _draw_patterns_in_napari(
                self.viewer,
    ...
    def update_ui(self):
        if self.image_widget is not None:
            if self.image_widget.ib_image is not None:
                ...
                if (
                    self.show_lamella.isChecked()
                    or self.checkBox_show_trench.isChecked()
                    or self.checkBox_show_notch.isChecked()
                ):
                    self.draw_patterns()
        ...
        if self.experiment.positions == []:
            ...
            current_lamella = self.experiment.positions[
                self.lamella_index.currentIndex()
            ]
            if current_lamella.state.stage in [
                AutoLamellaWaffleStage.Setup,
            ...
            elif current_lamella.state.stage in [
                AutoLamellaWaffleStage.ReadyTrench,
                AutoLamellaWaffleStage.ReadyLamella,
            ]:
        ...
        for lamella in self.experiment.positions:
            string_lamella += (
                f"Lamella {lamella._name}: \t\t{lamella.state.stage.name}\n"
            )
        self.lamella_count_txt.setPlainText(string_lamella)
    ...
    def go_to_lamella_ui(self):
        ...
        position = self.experiment.positions[
            index
        ].state.microscope_state.absolute_position
        self.microscope.move_stage_absolute(position)
        logging.info(
            f"Moved to position of lamella {self.experiment.positions[index]._name}."
        )
    ...
    def add_lamella_ui(self):
        ...
        self.experiment = add_lamella(
            microscope=self.microscope,
            experiment=self.experiment,
            ref_image=self.image_widget.ib_image,
        )
        ...
        if len(self.experiment.positions) > 1:
            ...
            fiducial_position = (
                ...
                if self.fiducial_position is not None
                    ...
                    -((self.image_widget.image_settings.resolution[0] / 3) * pixelsize),
                    0.0,
                )
            )
    ...
    def remove_lamella_ui(self):
        if (
            self.experiment.positions[self.lamella_index.currentIndex()].state.stage
            == AutoLamellaWaffleStage.MillFeatures
            and self.lamella_saved > 0
        ):
            self.lamella_saved -= 1

        name = self.lamella_index.currentText()
        self.experiment = remove_lamella(
            self.experiment, self.lamella_index.currentIndex()
        )
        self.lamella_index.clear()
    ...
    def save_lamella_ui(self):
        ...
        if response:
            ...
            lamella_position.x = (
                float(self.lamella_position.x)
                if self.lamella_position is not None
                else 0.0
            )
            lamella_position.y = (
                float(self.lamella_position.y)
                if self.lamella_position is not None
                else 0.0
            )
            ...
            if not validate_lamella_placement(
                self.settings.protocol,
                lamella_position,
                self.image_widget.ib_image,
                self.microexpansionCheckBox.isChecked(),
            ):
                _ = message_box_ui(
    ...
    def _clickback(self, layer, event):
        if event.button == 2:
            ...
            position = conversions.image_to_microscope_image_coordinates(
                coord=Point(coords[1], coords[0]),
                image=self.image_widget.ib_image.data,
                pixelsize=pixelsize,
            )
            ...
            if self.comboBox_moving_pattern.currentText() == "Fiducial":
                ...
                area, flag = calculate_fiducial_area(
                    self.settings, position, fiducial_length, pixelsize
                )
                if flag:
                    show_error(
                        "The fiducial area is out of the field of view. Please move fiducial closer to centre of image."
                    )
                    return
                if (
                    len(self.experiment.positions) != 0
                    and self.experiment.positions[
                    ...
                    self.experiment.positions[
                        int(self.lamella_index.currentIndex())
                    ].fiducial_centre = deepcopy(position)
                else:
            ...
            if self.comboBox_moving_pattern.currentText() == "Lamella":
                ...
                if not validate_lamella_placement(
                    ...
                    show_error(
                        "The lamella is out of the field of view. Please move lamella closer to centre of image."
                    )
                    return
                if (
                    len(self.experiment.positions) != 0
                    ...
                    self.experiment.positions[
                        int(self.lamella_index.currentIndex())
                    ].lamella_centre = deepcopy(position)
                else:
            ...
            if self.comboBox_moving_pattern.currentText() == "Notch":
                ...
                if (
                    ...
                    self.experiment.positions[
                        int(self.lamella_index.currentIndex())
                    ].notch_centre = deepcopy(position)
    ...
    def save_current_position(self):
        ...
        if self.experiment.positions[index].state.stage is AutoLamellaWaffleStage.Setup:
            ...
            self.experiment.positions[
                index
            ].state.stage = AutoLamellaWaffleStage.ReadyTrench

            # get current ib image, save as reference
            fname = os.path.join(
                self.experiment.positions[index].path, "ref_position_ib"
            )
            self.image_widget.ib_image.save(fname)

        elif (
            self.experiment.positions[index].state.stage
            is AutoLamellaWaffleStage.ReadyTrench
        ):
    ...
    def run_waffle_trench(self):
        ...
        self.experiment = wfl.run_trench_milling(
    ...
    def run_waffle_notch(self):
        ...
        self.experiment = wfl.run_notch_milling(
            microscope, microscope_settings, experiment, parent_ui=self
        )
    ...
    def run_autolamella_v2(self):
        ...
        self.experiment = wfl.run_lamella_milling(
            microscope, settings, experiment, parent_ui=self
        )
...
def add_lamella(
    microscope: FibsemMicroscope, experiment: Experiment, ref_image: FibsemImage
):
    """
...
def save_lamella(
    ...
    fiducial_area, flag = calculate_fiducial_area(
        microscope_settings, fiducial_centre, fiducial_length, pixelsize
    )
    if flag:
        _ = message_box_ui(
...
def mill_fiducial(
    ...
    try:
        ...
        lamella.path = os.path.join(
            lamella.path,
            f"{str(lamella.lamella_number).rjust(2, '0')}-{lamella._petname}",
        )
...
def run_autolamella(
    ...
    _microexpansion_used = any(
        [
            stage
            for stage in lamella_stages
            if stage.name == AutoLamellaWaffleStage.MicroExpansion.name
        ]
    )
    success = True
    lamella: Lamella
    for i, stage in enumerate(lamella_stages):
        ...
        for j, lamella in enumerate(experiment.positions):
            ...
            if curr_stage is AutoLamellaWaffleStage.RoughCut:
                if (
                    not _microexpansion_used
                ):  # , check if last stage was fiducial milled
            ...
            for _ in range(
                ...
                if current_alignment:
                    if isinstance(microscope, ThermoMicroscope) or isinstance(
                        microscope, DemoMicroscope
                    ):
                        microscope.set(
                            "current", stage.milling.milling_current, BeamType.ION
                        )
...
def main():
    ...
    widget = window.viewer.window.add_dock_widget(
        window, area="right", add_vertical_stretch=True, name="Autolamella"
    )
