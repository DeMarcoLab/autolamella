...
# TODO: thread this
def run_spot_burn_workflow(microscope: FibsemMicroscope, experiment: Experiment, parent_ui: AutoLamellaUI):

    for position in experiment.positions:

        print(f"Running spot burn workflow for {position.name}")
        # move to position
        stage_position = position.state.microscope_state.stage_position

        # convert position to FIB orientation
        stage_orientation = microscope.get_stage_orientation(stage_position)

        if stage_orientation == "UNKNOWN":
            logging.warning(f"Stage orientation is unknown for {position.name}.")
            continue

        # imitate compucentric movements
        if (stage_orientation in ["SEM", "MILLING"]):
            # convert to FIB orientation
            pos = microscope.get_orientation("FIB")
            rotation, tilt = pos.r, pos.t
            fib_stage_position = deepcopy(stage_position)
            fib_stage_position.x = -stage_position.x
            fib_stage_position.y = -stage_position.y
            fib_stage_position.r = rotation
            fib_stage_position.t = tilt
        if stage_orientation == "FIB":
            fib_stage_position = deepcopy(stage_position)
        
        microscope.safe_absolute_stage_movement(fib_stage_position)

        ret = ask_user(parent_ui=parent_ui,
                msg=f"Run the spot burn workflow for {position.name}. Press continue when finished.",
                pos="Continue", neg="Exit")
