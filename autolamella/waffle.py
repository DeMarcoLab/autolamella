...
def mill_trench(
    ...
    stages = patterning._get_milling_stages(
        "trench", settings.protocol, point=lamella.trench_centre
    )
...
def end_of_stage_update(
    ...
    logging.info(f"STATUS | {lamella._name} | {lamella.state.stage.name} | FINISHED")
...
def start_of_stage_update(
    ...
    logging.info(f"STATUS | {lamella._name} | {lamella.state.stage.name} | STARTED")
